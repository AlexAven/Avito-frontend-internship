...
- formik - создание и управления формами;
- yup - валидация форм;
...
Пока статус запроса является pending пользователю будет отрисован спиннер загрузки. При удачном завершении запроса пользователю отобразятся карточки объявлений либо уведомление об отсутствии объявлений. В случае ошибки пользователь увидит компонент ошибки с текстовым сообщение. 
...
- Поисковая строка ищет совпадения объявлений по их названию. 
...
Данные нового объявления вводятся в форму содержащую обязательные не обязательные строки и выпадающие меню которые позволяют ввести только валидные данные и в случае ошибки укажут на нее и не пропустят к следующему шагу.
(форма и валидация реализованы с помощью formik + yup + кастомные CSS стили). Если не указать URL-ссылку на фото, то отобразится заглушка. При завершении создания объявления вы будете перенаправлены на главную страницу и получите всплывающее уведомление о удачном или не удачном создании нового объявления. Созданное объявление отправится на сервер и отобразится в общем списке в реальном времени. 

- Карточки объявлений содержат краткую информацию о товаре и фотографию. В случа ее отсутствия буде показана заглушка. 
...
- При переходе к редактированию объявления вы перейдете на страницу "/form" для изменения информации товара. О том что это редактирование, а не создание нового товара вам подскажет динамический заголовок формы. Так как первый шаг формы содержал в себе данные не подлежащие редактированию – вы автоматически попадете на второй шаг объявления, где можете ввести новые данные. Текущие данные при этом остаются предзаполненными в форме. Ввести можно только валидные данные (валидация здесь также работает). После внесения изменений и подтверждения вы будете перенаправлены на главную страницу и получите всплывающее уведомлению об удачно или не удачном редактировании объявления. Внесенные изменения будут отправлены на сервер и изменятся на сайте в реальном времени.
...
- Выполнить в терминале команду `make install` в корне проекта (установит зависимости проекта и сервера);
